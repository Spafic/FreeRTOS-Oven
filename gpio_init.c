...
void InitGPIO(void) {
    ...
    // Port B: I2C (PB2, PB3) and PWM (PB6)
    // Configure I2C pins first
    ...
    // Configure PWM pin PB6 for alternate function before enabling as digital output
    GPIO_PORTB_AFSEL_R |= (1 << 6);              // Enable alternate function for PB6
    GPIO_PORTB_PCTL_R = (GPIO_PORTB_PCTL_R & ~0x0F000000) | 0x04000000; // PWM function for PB6
    ...
    // PB6 will be controlled by PWM hardware (set as digital output after AF/PCTL set)
    GPIO_PORTB_DEN_R |= ((1 << 2) | (1 << 3) | (1 << 6)); // Enable digital functions
}
